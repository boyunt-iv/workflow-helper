(function (root) {
  ...
  function extractBridgeFunctionSource(raw) {
    const text = String(raw || "");
    const start = text.indexOf("function liveBridge");
    if (start < 0) return text;
    const braceStart = text.indexOf("{", start);
    if (braceStart < 0) return text.slice(start);
    let depth = 0;
  ...
  function buildBridgeBookmarklet(raw) {
    return "javascript:(" + extractBridgeFunctionSource(raw)
      .replace(/^\s*\/\/.*$/gm, "")
      .replace(/\n\s*/g, " ")
      .replace(/\s{2,}/g, " ")
      .trim() + ")();void 0;";
